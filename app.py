...
def _derive_gradebook(evaluation: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Derive simple gradebook entries from an evaluation dict returned by the LLM.
    Returns { entries: [...], totals: {earned, total} }.
    - Fallbacks: if points missing, default points=1; if points_earned missing, infer from correctness.
    """
...
def _deepseek_ocr_from_bytes(img_bytes: bytes, language: str = "vi") -> Optional[str]:
    """Thin wrapper around deepseek_ocr. Returns string or None."""
    try:
...
async def extract_score(submission_image: UploadFile = File(...), language: str = Form("vi")):
    ...
    try:
        ...
        try:
            ...
            data = {"raw": text}
        # Add derived gradebook for clearer scoring breakdown
        try:
            gradebook = _derive_gradebook(data)
            if isinstance(data, dict):
                data["gradebook"] = gradebook
        except Exception:
            pass
        return data
    finally:
...
@app.post("/ocr/deepseek-extract")
async def deepseek_extract(submission_image: UploadFile = File(...), language: str = Form("vi")):
    """Use DeepSeek (if configured) to OCR the student's submission. Returns JSON with lines."""
    try:
        content = await submission_image.read()
    except Exception:
        raise HTTPException(400, "Cannot read uploaded file")
    text = _deepseek_ocr_from_bytes(content, language=language)
    if text is None:
        raise HTTPException(500, "DeepSeek OCR unavailable or failed. Ensure DEEPSEEK_API_KEY is set.")
    lines = [s for s in str(text).splitlines() if s.strip()]
    return {"lines": lines, "char_count": sum(len(s) for s in lines)}
...
async def grade_from_images(
    ...
    try:
        ...
        # Optional DeepSeek OCR hint from submission
        ds_ocr_hint = _deepseek_ocr_from_bytes(sub_bytes, language=language)

        prompt = {
            "task": "grade_submission_against_exam",
            "instructions": [
                "You are a strict, consistent grader.",
                "FIRST, parse the exam image into a hierarchical outline: big questions 'Bài 1, Bài 2, ...' and subparts 'a), b), c)...'.",
                "For each leaf subpart, create a normalized label like 'B1.a', 'B2.c' (ASCII only).",
                "Include a concise 'question' text per leaf using OCR, keeping math readable without LaTeX.",
                "SECOND, read the student's submission and map each written answer to the corresponding leaf label.",
                "If mapping is uncertain, set mapping_confidence < 0.6 and add a short note.",
                "Judge correctness (prefer visible ticks/crosses; otherwise reason briefly).",
                "If no points are printed, default to 1 point per leaf subpart.",
                "Return strictly valid JSON using the schema below. Be concise in rationales.",
            ],
            "ocr_hint": ds_ocr_hint,
            "output_schema": {
                "type": "object",
                "properties": {
...
<body>
  ...
  function rHints(list){
    if(!Array.isArray(list)||!list.length) return ''
    const rows = list.map(it=>`<tr><td>${esc(it.label||'')}</td><td>${(it.hints||[]).map(esc).join('<br>')}</td></tr>`).join('')
    return `<h3>Gợi ý theo từng mục</h3><table><thead><tr><th>Mục</th><th>Gợi ý</th></tr></thead><tbody>${rows}</tbody></table>`
  }
  ...
  f.addEventListener('submit', async (e)=>{
    ...
    document.getElementById('hints').innerHTML = rHints(js.hint_questions);
...
async def agent_suggest_questions(
    ...
    try:
                        ...
                        "items": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "rubric": {
                                        "type": ["array","null"],
                                        "items": {"type": "object","properties": {"criterion": {"type": "string"}, "max_points": {"type": "number"}, "earned": {"type": ["number","null"]}}, "required": ["criterion","max_points"], "additionalProperties": False}
                                    }
                                },
                                "required": ["label"],
                                "additionalProperties": False
        ...
        # Derive gradebook entries from evaluation for clear per-part scoring
        gradebook = _derive_gradebook(evaluation)
        ...
        if support_plan:
            ...
            gen_prompt = {
                ...
                "instructions": [
                    ...
                    "If the skill is geometry (hình học), include a simple self-contained inline SVG diagram in 'diagram_svg' (viewBox='0 0 300 200'), draw points/segments/arcs, and label key points (A,B,C,...) with <text>. Keep it minimal and readable.",
                    "If you cannot produce an SVG, add a short 'diagram_description' describing the setup.",
        ...
        # Generate per-part guiding questions (hints) to help reach the goal
        ...
        if wrong_items:
            target_hints = max(3, min(8, len(wrong_items)))
            hints_prompt = {
                ...
                "output_schema": {
                    ...
                    }, "required": ["label","hints"], "additionalProperties": False}
                },
                "locale": language
            }
            try:
                hresp = model.generate_content([json.dumps(hints_prompt)])
                htext = getattr(hresp, "text", None) or "[]"
                hint_questions = json.loads(htext)
    ...
    try:
        ...
        prompt = {
            ...
            "instructions": [
                ...
                "If the topic is geometry (hình học), embed a minimal inline SVG diagram in 'diagram_svg' (viewBox '0 0 300 200'), draw lines/circles/arcs and label points with letters.",
                "If you cannot produce an SVG, include a short 'diagram_description'.",
                "Return strictly valid JSON only.",
            ],
        ...
        if support_plan:
            gen_prompt = {
                ...
                "instructions": [
                    ...
                    "If geometry (hình học), include a small inline SVG diagram in 'diagram_svg' (viewBox '0 0 300 200'), label points with letters. Otherwise leave null.",
